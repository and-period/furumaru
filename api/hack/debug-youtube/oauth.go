...
func getService(ctx context.Context) (*youtube.Service, error) {
	b, err := os.ReadFile("./tmp/furumaru-stg-youtube.json")
	if err != nil {
	...
	return youtube.NewService(ctx, option.WithTokenSource(config.TokenSource(ctx, token)))
}
...
func getToken(config *oauth2.Config) (*oauth2.Token, error) {
	...
	if launchWebServer {
		log.Println("Trying to get token from web")
		token, err = getTokenFromWeb(config, authURL)
	} else {
		log.Println("Trying to get token from prompt")
...
func getTokenFromPrompt(config *oauth2.Config, authURL string) (*oauth2.Token, error) {
	var code string
	log.Printf("Go to the following link in your browser. After completing "+
...
func getTokenFromWeb(config *oauth2.Config, authURL string) (*oauth2.Token, error) {
	...
	if err != nil {
		log.Printf("Unable to start a web server.")
		return nil, err
	}
