...
func TestProductTypeIcon(t *testing.T) {
	t.Parallel()
			...
			setup: func(ctx context.Context, mocks *mocks) {
				...
				mocks.store.EXPECT().
					...
					DoAndReturn(func(ctx context.Context, in *store.UpdateProductTypeIconsInput) error {
						assert.Equal(t, "target-id", in.ProductTypeID)
						assert.ElementsMatch(t, icons, in.Icons)
						return nil
					})
			},
			payload: &entity.ResizerPayload{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, testResizer(tt.setup, func(ctx context.Context, t *testing.T, resizer *resizer) {
			t.Parallel()
			err := resizer.productTypeIcon(ctx, tt.payload)
