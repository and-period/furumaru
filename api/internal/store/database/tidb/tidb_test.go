...
func newTestDBClient(ctx context.Context) (*mysql.Client, func(), error) {
	...
	client, cleanup, err := mysql.NewContainerDB(ctx,
		mysql.WithContainerDatabase("stores"),
...
// infra/tidb/schema/stores/ を解決する。
func schemaDir() (string, error) {
	...
	dir := filepath.Join(repoRoot, "infra", "tidb", "schema", "stores")
...
func deleteAll(ctx context.Context) error {
	tables := []string{
		...
		liveTable,
		scheduleTable,
		experienceReviewReactionTable,
		experienceReviewTable,
		experienceRevisionTable,
		experienceTable,
		experienceTypeTable,
		productReviewReactionTable,
		productReviewTable,
