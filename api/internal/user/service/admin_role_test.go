...
func TestGenerateAdminRole(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "success",
			setup: func(ctx context.Context, mocks *mocks) {
				mocks.db.AdminPolicy.EXPECT().List(gomock.Any(), policiesParams).Return(policies, nil)
				mocks.db.AdminRolePolicy.EXPECT().List(gomock.Any(), rolePoliciesParams).Return(rolePolicies, nil)
				mocks.db.AdminGroupRole.EXPECT().List(gomock.Any(), groupRolesParams).Return(groupRoles, nil)
			},
			input:       &user.GenerateAdminRoleInput{},
...
`,
		...
		},
		{
			...
			setup: func(ctx context.Context, mocks *mocks) {
				mocks.db.AdminPolicy.EXPECT().List(gomock.Any(), policiesParams).Return(nil, assert.AnError)
				mocks.db.AdminRolePolicy.EXPECT().List(gomock.Any(), rolePoliciesParams).Return(rolePolicies, nil)
				mocks.db.AdminGroupRole.EXPECT().List(gomock.Any(), groupRolesParams).Return(groupRoles, nil)
			},
			input:        &user.GenerateAdminRoleInput{},
		...
		},
		{
			...
			setup: func(ctx context.Context, mocks *mocks) {
				mocks.db.AdminPolicy.EXPECT().List(gomock.Any(), policiesParams).Return(policies, nil)
				mocks.db.AdminRolePolicy.EXPECT().List(gomock.Any(), rolePoliciesParams).Return(nil, assert.AnError)
				mocks.db.AdminGroupRole.EXPECT().List(gomock.Any(), groupRolesParams).Return(groupRoles, nil)
			},
			input:        &user.GenerateAdminRoleInput{},
			expectModel:  "",
		...
		},
		{
			...
			setup: func(ctx context.Context, mocks *mocks) {
				mocks.db.AdminPolicy.EXPECT().List(gomock.Any(), policiesParams).Return(policies, nil)
				mocks.db.AdminRolePolicy.EXPECT().List(gomock.Any(), rolePoliciesParams).Return(rolePolicies, nil)
				mocks.db.AdminGroupRole.EXPECT().List(gomock.Any(), groupRolesParams).Return(nil, assert.AnError)
			},
			input:        &user.GenerateAdminRoleInput{},
			expectModel:  "",
			expectPolicy: "",
