...
// Build initializes the user gateway and returns the router
...
func (a *app) Build(ctx context.Context) (*BuildResult, error) {
	...
	if err := envconfig.Process("", a); err != nil {
		return nil, fmt.Errorf("user: failed to load environment: %w", err)
	}

	// 依存関係の解決
	if err := a.inject(ctx); err != nil {
		return nil, fmt.Errorf("user: failed to new registry: %w", err)
	}
