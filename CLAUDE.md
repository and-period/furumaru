...
### /save コマンド
`/save` コマンドが実行された場合、以下のように学習内容を保存すること：

#### セッション記録（.claude/debug/）
- 作業セッションの詳細を `YYYY-MM-DD_[作業内容].md` 形式で保存
- 実施したタスクの概要、発見した問題と解決策、修正したファイルの詳細
- エラーとその解決過程、学習したパターンや知見
- 今後の注意点や改善提案

#### 知識の蓄積（.claude/knowledge/）  
- 新しく理解したアーキテクチャパターンや設計思想
- ビジネスロジックの整理、API仕様の理解
- 開発・運用プロセスのベストプラクティス
- 既存知識の更新・補完
...
2. 学んだ知識を .claude/knowledge/ に整理保存
3. 既存の知識ファイルの更新
```
