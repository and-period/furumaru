...
| 要素 | 現状 | 問題 | 対応 |
|------|------|------|------|
| `typography: #707070` on `base: #F9F6EA` | コントラスト比 4.1:1 | AA 基準ギリギリ | `#5a5a5a` 以上に変更（4.5:1 以上） |
| `orange: #F48D26` on white | コントラスト比 2.6:1 | 不合格 | ボタンテキストは白以外に変更、または背景色を調整 |
...
### 実装開始前
* [ ] Core Web Vitals のベースライン計測（Lighthouse CI）
